...
UVO is a javascript universal single object validation library. The main goal is to provide schema creation tool for convenient and flexible form's validation with the only one call. Schema can represent either simple form with few fields or huge one with a big amount of cross form validations, recursive descents, different conditions and custom data processing.
...
UVO bundles:
  - Classic API: `esm ~7.3kb`, `cjs ~7.5kb`, `umd ~10.4kb`
  - Templating Api: `esm ~8.7kb`
...
In the example above you can see not only validators (namely `trim`). Library has `processor` modules those only transform value into another. You can inject your custom processors and validators as we did for roles filtration.
...
In the above schema we use `setDep` and `getDep` (i.e. referencing) to compare fields to each one.
In that case we must use `withMeta` container. It provides global data storage for scheme, allows us to use references and gives meta data for errors.
