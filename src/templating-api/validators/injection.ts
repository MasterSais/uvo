...
import { l_injections, l_isFunction, l_quoted, l_square } from '@lib/templating-api/units';
...
export const injectionTemplate = (props: CompilerProps, data: ValidatorData): Array<string> => {
  ...
  const param = data.params ? extract(props.components, data.params[0])({ ...props, internal: true }, data.params[0]).join('') : '';

  return ([
    `(${l_isFunction(injection)}?${injection}(${param}):${injection})`
  ]);
};