...
import { l_and, l_compare, l_content, l_else, l_embrace, l_emptyString, l_equal, l_error, l_if, l_ifBody, l_indexOf, l_isNumber, l_length, l_not, l_notEqual, l_null, l_or, l_test, l_typeof, l_undefined } from '@lib/templating-api/compiler/units';
  ...
  for (let i = 0; i < data.params.length; i += 3) {
    conditions.push(
      (
        ...
        props.in,
        extract(props.components, data.params[i + 1])().join('')
      )
    );
  }
